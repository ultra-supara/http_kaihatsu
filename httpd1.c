...
// HTTPRequestはLinuxにおけるきゃっとみたいなやつという認識でいいと思う
struct HTTPRequest {
...
static void service(FILE *in, FILE *out, char *docroot) {
    ...
    //inからHTTPリクエストを読んでその情報をstruct HTTPRequest に格納して返す
    req = read_request(in);
    // HTTPリクエストreqに対するレスポンスを第二引数outのストリームに書きこむ
...
//HTTPリクエストラインを第2引数から読み込んで内容を解析して第1引数のreqに書き込む
static void read_request_line(struct HTTPRequest *req, FILE *in) {
...
//struct HTTPRequest からリクエストのエンティティボディの長さを取得する
static long content_length(struct HTTPRequest *req) {
    char *val;
    long len;
...
//Get Requestの処理 HTTPにおけるcatコマンド
static void do_file_response(struct HTTPRequest *req, FILE *out, char *docroot) {
